...
class AIService:
    ...
    def smart_process_command(cls, user_text: str, branch: str, persona: str = "hantouf", 
        ...
        1. دعم المعاملات المركبة واستعلامات الحد الائتماني:
           - إذا طلب التاجر تعيين أو تعديل حد ائتماني لعميل (مثل: "حط حد ائتماني للعميل أحمد 15000" أو "خلي حد ائتمان محمود 20000")، اجعل نوع المعاملة "UPDATE_CREDIT_LIMIT".
           - استخرج اسم العميل في `supplier` أو `item_name`، وقيمة الحد الائتماني الرقمية الجديدة في `unit_price` أو حقل مخصص.

        2. تصنيف أنواع المعاملات (`type`):
           - "PURCHASE": شراء أو توريد للمخزن.
           - "SALE": بيع أو خروج من المخزن.
           - "RETURN": مرتجع مشتريات أو مبيعات.
           - "QUERY": استعلام عن رصيد أو ديون.
        ...
        {{
            ...
            "transactions": [
                {{
                    ...
                    "item_name": "اسم الصنف أو اسم العميل في حالة الحد الائتماني",
